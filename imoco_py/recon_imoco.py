...
import sigpy as sp
import numpy as np
from sigpy_e import cfl, ext, reg
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--res_scale", type=float, default=1, help="scale of resolution, full res == 1"
    )
    ...
    parser.add_argument("--n_ref", type=float, default=-1, help="reference frame, default is -1.")
    parser.add_argument(
        "--reg_flag", type=int, default=0, help="derive motion field from registration"
    )

    parser.add_argument(
        "--lambda_TV", type=float, default=5e-2, help="low rank regularization, default is 0.05"
    )
    parser.add_argument("--iner_iter", type=int, default=15, help="Num of inner iterations.")
    ...
    parser.add_argument("fname", type=str, help="Prefix of raw data and output(_mocolor).")
    ...
    for i in range(nphase):
        ...
        FTSM = W * FTs * S * M
        PFTSMs.append(FTSM)
    PFTSMs = Diags(PFTSMs, oshape=(nphase, nCoil, npe, nfe,), ishape=(nphase,) + tshape) * Vstacks(
        Is, ishape=tshape, oshape=(nphase,) + tshape
    )
